...
| 오전 잡담 | KST 09:00~12:05, 하루 6~8건 텍스트 (chat.yml, 기본 비활성) |
| 답글 | KST 12:19 / 16:53 / 22:07 전부 + 잡담 실행마다 스윕. 일 20건·저자 2건 상한 |
...
## 5-A. 오전 잡담(CHAT) 운영 (v1.1)

### 활성화 순서
1. `Actions → Threads Chat → Run workflow → dry_run` 수동 실행 (CHAT_ENABLED=false 여도 수동 dry_run 은 동작)
2. 로그에서 `소스=`·`테마=`·`--- CHAT 본문 ---` 확인. `소스=none` 이면 RSS/웹 검색 모두 실패
3. Variables `CHAT_ENABLED=true`, 1주차 `CHAT_DAILY_MIN=3`·`CHAT_DAILY_MAX=4` 권장 → 이후 단계 증량

### 로그 읽는 법
| 로그 | 의미 |
|---|---|
| `T n(미선택) 까지 허용 k건 — 이미 k건 발행` | 정상. 계획대로 나가 있음. 답글 스윕만 수행 |
| `간격 확보를 위해 최소 N초 대기` | 정상. 직전 글과 15분을 맞추려 기다렸다 발행 |
| `간격 확보에 N초 대기 필요 — 한도 600초 초과` | 직전 글이 너무 가까움. 이번 트리거는 보류, 다음 트리거가 보충 |
| `목표 N건 도달` | 정상 |
| `CHAT 린트 실패` 반복 | 생성문이 숫자·기업명·전망 표현을 씀. 2회 소진 시 그 트리거는 생략 |
...
| `TOKEN_REFRESH_THRESHOLD_DAYS` | `10` | 갱신 임계. 잔여가 이 값 이하일 때만 갱신 |
